...
if [[ ! -f "$MA_DIR/workspace/test.txt" ]]; then
  printf 'smoke_test: missing multi_agent/workspace/test.txt\n'
  FAILURES=$((FAILURES + 1))
fi
...
python3 "$SCRIPT_DIR/write_memory.py" \
  ...
  --files-changed "multi_agent/workspace/test.txt" \
  --commands-run "bash multi_agent/scripts/run_workflow.sh --type modify --goal create workspace/test.txt with hello multi-agent --backend shell" \
